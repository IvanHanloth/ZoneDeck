...
# 解除安裝程式簽章（發版流程用）：先產生待簽的 dist/uninstaller/uninst-*.e32，簽完章再打安裝包
...
授權條款頁之後是一頁隱私權聲明（Inno 的 `InfoBeforeFile`，依安裝語言各取一份）。正文以文件站的三份 Markdown 為唯一真源：

| 語言      | 原文                      | 產生的純文字                                                       |
|---------|-------------------------|--------------------------------------------------------------|
| 簡體中文    | `docs/privacy.md`       | `.github/inno-script/privacy/PRIVACY.chinesesimplified.txt`  |
...
轉換由 `scripts/gen-privacy.ps1` 完成（`package.ps1` 在編譯安裝包前自動呼叫），產物不入庫。要改隱私權聲明只動文件站那三份，手抄一份到安裝腳本裡遲早和網站對不上。

::: warning 直接呼叫 ISCC 會編譯失敗
純文字不入庫，略過 `package.ps1` 直接跑 `ISCC` 時這三個檔案不存在。請先執行 `powershell -File scripts/gen-privacy.ps1`。
:::
...
5. `package.ps1 -SkipFrontend` 組裝 `dist/ZoneDeck` → `package.ps1 -PrepareUninstaller` 產生待簽的解除安裝程式 → 為兩個
   exe 與解除安裝程式加上程式碼簽章 → `package.ps1 -InstallerOnly -SignedUninstaller` 把它們打進安裝包 → 單獨簽安裝包本身 →
   把 `dist/ZoneDeck` 壓成可攜 zip；
...
::: tip 解除安裝程式為什麼要單獨簽
`unins000.exe` 是編譯安裝包時嵌進去、安裝時才產生的，事後簽 `Setup.exe` 簽不到它。Inno 的外部簽章流程是先編譯出 `uninst-*.e32`（首次編譯就此中止），簽好章再編譯一次，簽章隨安裝包寫進 `unins000.exe`。
:::
