...
前端在編譯期被**內嵌**進 `bosskey-config.exe`，最終產物靜態執行。開發前端時用 `npm run dev` 在瀏覽器裡預覽（mock 資料、熱重新載入），改完 `npm run build` 後用 `cargo run -p bosskey-config` 驗證 Tauri 整合。
:::
...
# 一鍵生產打包（前端 + Rust + 可攜資料夾 dist/Boss-Key）
...
| 目的 | 命令 |
| --- | --- |
| 執行核心 | `cargo run -p bosskey-core` |
| 核心冒煙自測 | `cargo run -p bosskey-core -- smoke 3000` |
...
| 執行設定介面 | `npm --prefix apps/config/ui run build && cargo run -p bosskey-config` |
...
::: tip 核心測試需單執行緒執行
部分核心測試涉及 COM 初始化，多執行緒並行可能導致當機。若遇到此類問題，使用 `cargo test -p bosskey-core -- --test-threads=1` 單執行緒執行。詳見 [測試策略](/zh-tw/dev/testing)。
:::
