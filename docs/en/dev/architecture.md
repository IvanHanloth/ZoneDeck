...
│           effects.rs    Effects trait (muting/freezing/pausing media; mockable)
│           effects_worker.rs  Dedicated side-effect thread (FIFO queue; the message loop only does SW_HIDE)
│           audio.rs      Core Audio session muting (per-session bookkeeping) + playback check
│           media.rs      SMTC media sessions (per-app pause; falls back to the global key on timeout)
...
When hiding or showing is triggered, `HideController` orchestrates it with a two-phase, intent-first flow: `plan_hide` computes the execution plan (pruning stale records and backfilling PIDs) → the planned snapshot is written to `recovery.json` (persist first, act second — a crash mid-hide loses no records) → `commit_hide` hides the windows synchronously (`SW_HIDE`) and hands muting / freezing / pausing media to the dedicated side-effect thread (`effects_worker.rs`), executed asynchronously in FIFO order — the message loop is never blocked by slow operations (audio enumeration, waiting on pssuspend), so hotkeys and the UI stay responsive.

The order within the queue matters: pause media → mute → settle → freeze. Freezing stops a process from responding to messages at all, so freezing before the hide has finished painting leaves a ghost of the window on screen; the pause command likewise needs time to be handled by the target program. Hence a single settle before the batch of freezes (`FREEZE_SETTLE_DELAY`, once per batch, skipped when there is nothing to freeze). Muting is deliberately not placed behind that wait — it goes through the audio session and does not care whether the target process is running.

Pausing media takes two routes. First, SMTC (`media.rs`) issues an explicit pause to individual sessions, touching only those that belong to a hidden process and are actually playing. Targets it cannot match (no registered SMTC session, or an AUMID that does not line up with the image path) fall back to the global media key — and only once those targets are confirmed to be making sound, because that key is a play/pause **toggle**: sent while nothing is playing, it starts playback instead. Waiting on SMTC blocks indefinitely, so the whole call is handed to a temporary thread with a deadline; on timeout it is treated as "nothing was paused", which keeps it from holding up the resumes queued behind it.

When restoring (showing), every record is validated first: the handle must still exist and still belong to the original process (`IsWindow` + PID comparison), and frozen / efficiency-mode records must match the process creation time — both handles and PIDs are recycled by the system, and records that fail validation are skipped and reported truthfully in the log.

Muting is the exception: it is **not** gated on the process still being alive. Muting reaches every session of the same image, so sessions belonging to other processes of that executable still need unmuting even after the target itself has exited — they are found again through the image path stored in the record. Mistakes are held off by the per-session bookkeeping in `audio.rs`: only sessions this program actually flipped from unmuted to muted are ever unmuted, so a mute the user set in the volume mixer stays put. Efficiency mode records its prior state the same way: a process that already had EcoQoS explicitly set keeps it when the mode is lifted. Nothing is paused or resumed on restore — deciding whether playback should continue is left to the user.
