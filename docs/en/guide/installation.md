...
| --- | --- | --- |
| **installer** | A full installer providing one-click install, update and uninstall | **Recommended**. Manages ZoneDeck more efficiently and safely |
...
1. Download `ZoneDeck-<version>-Setup.exe`.
...
3. ZoneDeck starts automatically once installation finishes and opens the settings window.

The installer first asks who it is installing for:

- **Install for me only** (default, no administrator rights needed): installs into `%LocalAppData%\Programs\ZoneDeck`.
- **Install for all users** (needs administrator rights): installs into `C:\Program Files\ZoneDeck`.

Either way the settings are stored in `%APPDATA%\ZoneDeck`, not in the installation folder — see [Where the data lives](#where-the-data-lives) below. The installer drops an `installed.marker` file in the installation folder so the program knows it is an installed copy; do not delete it.

## Using the portable edition

1. Download `ZoneDeck-<version>-portable.zip`.
2. Extract it anywhere. The archive already contains a `ZoneDeck` folder — move that whole folder wherever you want it.
3. Run **`ZoneDeck.exe`** from that folder. On first run it opens the settings window automatically.

The extracted folder looks like this:

```
ZoneDeck/
├── ZoneDeck.exe      Resident core (runs in the background; hides windows / listens for hotkeys)
...
`ZoneDeck.exe` and `config.exe` cooperate through a shared `config.json` and a named pipe. Do not separate them.
:::
...
- **Installer**: **`%APPDATA%\ZoneDeck`**. The installation folder may be `C:\Program Files`, which normal privileges cannot write to, so settings kept there would fail to save every time.
...
ZoneDeck uses a two-process design that separates **core and settings**:

- **`ZoneDeck.exe` (core)**: resident in the background; listens for hotkeys and hides / shows windows.
...
The script lists what it is about to delete and waits for your confirmation, then removes `%LOCALAPPDATA%\cn.hanloth.zonedeck.config`, any `%APPDATA%\ZoneDeck` (present only if the program folder was not writable), and what autostart leaves behind: the scheduled task `ZoneDeckAutostart` and the registry entry `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\ZoneDeck Application`. The program folder itself is left alone — delete it yourself once the script is done.
